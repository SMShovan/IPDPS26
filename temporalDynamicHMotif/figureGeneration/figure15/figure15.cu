...
constexpr int kTHyMeThreadsPlaceholder = 8;
...
int nextMultipleOf4(int x) { return ((x + 3) / 4) * 4; }
...
void constructLayer(TemporalHypergraphIndex& index, TemporalLayer layer, const TemporalHostState& state, int h2vNumRecords, int v2hNumRecords) {
...
UpdateBatch buildSyntheticUpdateBatch(const std::vector<std::vector<int>>& originalH2V, int maxVerticesPerHyperedge, int minVertexId, int maxVertexId,
                                      int totalChanges) {
    UpdateBatch batch;
    ...
    batch.generatedInserts = hyperedge2vertex(numInserts, std::max(1, maxVerticesPerHyperedge), std::max(1, minVertexId),
                                              std::max(minVertexId + 1, maxVertexId));
...
double runDynamicCUDA(const std::vector<std::vector<int>>& baseH2V, int totalChanges) {
...
bool writeTemporalDatasetForBaseline(const std::vector<std::vector<int>>& h2v, const std::string& outPath) {
...
bool ensureBaselineCUDABinary() {
    const std::string cmd = "cd \"../../../THyMe-main/code\" && nvcc -O2 -std=c++17 main_thymeP_cuda.cu -o run_thymeP_cuda";
...
bool parseRuntimeSeconds(const std::string& stdoutText, double& seconds) {
...
double runBaselineCUDA(const std::vector<std::vector<int>>& baseH2V, int totalChanges, const std::string& datasetLabel, const std::string& changedLabel) {
    int maxVerticesPerHyperedge = 1;
    int minVertexId = INT_MAX;
    int maxVertexId = 0;
    for (const auto& row : baseH2V) {
    ...
    fs::create_directories("baseline_tmp");
    std::string safeLabel = datasetLabel;
    for (char& c : safeLabel) if (c == '/' || c == ' ') c = '_';
    const std::string graphPath = "baseline_tmp/" + safeLabel + "_" + changedLabel + ".txt";
    const std::string resultPath = "baseline_tmp/" + safeLabel + "_" + changedLabel + "_out.txt";
    if (!writeTemporalDatasetForBaseline(updatedH2V, graphPath)) return 0.0;

    std::ostringstream cmd;
    cmd << "cd \"../../../THyMe-main/code\" && "
        << "./run_thymeP_cuda dummy " << kTHyMeDelta << " " << kTHyMeThreadsPlaceholder
        << " \"../../../temporalDynamicHMotif/figureGeneration/figure15/" << graphPath << "\""
        << " \"../../../temporalDynamicHMotif/figureGeneration/figure15/" << resultPath << "\"";

    std::string out;
    if (!runCommandCapture(cmd.str(), out)) return 0.0;
    double runtime = 0.0;
    if (!parseRuntimeSeconds(out, runtime)) {
...
bool writeTimingTable(const std::vector<TimingRow>& rows) {
    std::ofstream out("cuda_thyme_speedup_timing.txt");
    if (!out.is_open()) return false;
...
int main() {
    if (!ensureBaselineCUDABinary()) {
        std::cerr << "Error: failed to compile CUDA baseline (THyMe)." << std::endl;
        return 1;
    }
    ...
    for (const auto& ds : kDatasets) {
        ...
        for (const auto& setting : kChangedEdgeSettings) {
            ...
            double baselineSeconds = runBaselineCUDA(h2v, totalChanges, ds.label, changedLabel);
            rows.push_back({"baselineCUDA", ds.label, changedLabel, baselineSeconds});

            double dynamicSeconds = runDynamicCUDA(h2v, totalChanges);
            rows.push_back({"dynamicCUDA", ds.label, changedLabel, dynamicSeconds});

            std::cout << ds.label << " changed=" << changedLabel
                      << " baselineCUDA=" << baselineSeconds
                      << " dynamicCUDA=" << dynamicSeconds << std::endl;
        }
    }

    if (!writeTimingTable(rows)) {
        std::cerr << "Error: failed to write cuda_thyme_speedup_timing.txt" << std::endl;
        return 1;
    }

    std::cout << "figure15 benchmarking complete." << std::endl;
    return 0;
}
