...
UpdateBatch buildSyntheticUpdateBatch(const std::vector<std::vector<int>>& originalH2V,
    ...
    const int effectiveChanges = std::min(kTotalChanges, numHyperedges);
    int numDeletes = static_cast<int>(std::llround(effectiveChanges * (deletePercentage / 100.0)));
...
double runESCHERTemporal(const std::vector<std::vector<int>>& baseH2V, int deletePercentage) {
    ...
    UpdateBatch batch = buildSyntheticUpdateBatch(baseH2V, maxVerticesPerHyperedge, minVertexId, maxVertexId, deletePercentage);
...
double runTHyMeBaseline(const std::vector<std::vector<int>>& baseH2V,
    ...
    const std::string graphPath = "thyme_tmp/" + safeLabel + "_del" + std::to_string(deletePercentage) + ".txt";
    const std::string resultPath = "thyme_tmp/" + safeLabel + "_del" + std::to_string(deletePercentage) + "_out.txt";
    ...
    cmd << "cd \"../../../THyMe-main/code\" && "
        ...
        << " \"../../../temporalDynamicHMotif/figureGeneration/figure13/" << graphPath << "\""
        << " \"../../../temporalDynamicHMotif/figureGeneration/figure13/" << resultPath << "\"";
...
bool writeTimingTable(const std::vector<TimingRow>& rows) {
    std::ofstream out("escher_vs_thymep_deletevary_temporal.txt");
    if (!out.is_open()) return false;
    out << "Mode Dataset DeletePercentage TimeSeconds\n";
    for (const auto& r : rows) out << r.mode << " " << r.dataset << " " << r.deletePercentage << " " << r.timeSeconds << "\n";
    return true;
}
...
int main() {
    ...
    for (const auto& ds : kDatasets) {
        ...
        for (int delPct : kDeletePercentages) {
            double thymeSeconds = runTHyMeBaseline(h2v, delPct, ds.plotLabel);
            rows.push_back({"THyME+", ds.plotLabel, delPct, thymeSeconds});

            double escherSeconds = runESCHERTemporal(h2v, delPct);
            rows.push_back({"ESCHER", ds.plotLabel, delPct, escherSeconds});

            std::cout << ds.plotLabel << " del=" << delPct
                      << " THyME+=" << thymeSeconds
    ...
    if (!writeTimingTable(rows)) {
        std::cerr << "Error: failed to write escher_vs_thymep_deletevary_temporal.txt" << std::endl;
        return 1;
    }

    std::cout << "figure13 benchmarking complete." << std::endl;
    return 0;
}
