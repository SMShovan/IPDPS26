...
def main() -> None:
    df = pd.read_csv("escher_vs_thymep_speedup_timing.txt", delim_whitespace=True)
    df = df.rename(
        columns={
    ...
    pivot_df.plot(kind="bar", ax=ax, width=0.9, color=sns.color_palette("Dark2"))
    ax.set_ylabel("Speedup (THyMe+ / ESCHER)")
    ...
    plt.tight_layout()
    plt.savefig("SpeedupTHyMe.pdf", format="pdf")
    plt.show()
