...
bool writeOutput(const std::vector<std::tuple<std::string, std::string, std::string, double>>& rows) {
    std::ofstream out("gpu_vs_gpu_timing.txt");
    if (!out.is_open()) {
...
int main() {
    ...
    for (const auto& ds : kDatasets) {
        ...
        for (const auto& changed : kChangedEdges) {
            ...
            double dynamicSeconds = std::chrono::duration<double>(t1 - t0).count() + std::chrono::duration<double>(t2 - t1).count() +
                                    std::chrono::duration<double>(t3 - t2).count();
            rows.push_back({"dynamicCUDA", ds.label, changed.second, dynamicSeconds});

            // baselineCUDA: full recomputation on changed hypergraph
            auto b0 = std::chrono::high_resolution_clock::now();
            DeviceGraphState baselineDevice = constructDeviceGraphState(updatedHost, maxId, maxId, "H2V-base", "V2H-base", "H2H-base");
            computeMotifCounts(baselineDevice.h2vOps.context(), baselineDevice.v2hOps.context(), baselineDevice.h2hOps.context(), maxId);
            auto b1 = std::chrono::high_resolution_clock::now();
            double baselineSeconds = std::chrono::duration<double>(b1 - b0).count();
            rows.push_back({"baselineCUDA", ds.label, changed.second, baselineSeconds});
        }
    }

    if (!writeOutput(rows)) return 1;
    std::cout << "figure10 benchmark complete." << std::endl;
    return 0;
}
