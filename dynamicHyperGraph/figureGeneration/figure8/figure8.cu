...
const std::vector<DatasetConfig> kDatasets = {
    {"Coauth-DBLP", "Coauth.txt"},
    {"Tags-stack-overflow", "Tags.txt"},
    {"com-Orkut", "Orkut.txt"},
    {"Threads-stack-overflow", "Threads.txt"},
    {"random", "Random.txt"},
};
const std::vector<int> kDeletePercentages = {20, 40, 60, 80};

constexpr int kPayloadCapacity = 1 << 20;
constexpr int kAlignment = 4;
constexpr int kTargetTotalChanges = 200000;
constexpr int kMoCHyThreads = 4;
...
bool writeMoCHyCsv(const std::vector<std::vector<int>>& h2v, const std::string& outputPath) {
    ...
    for (const auto& row : h2v) {
        ...
        for (size_t i = 0; i < row.size(); ++i) {
            if (i > 0) out << ",";
            // MoCHy expects node IDs starting from 0.
...
bool ensureMoCHyExecutable(const std::string& executablePath) {
    ...
    std::cout << "Compiling MoCHy-E executable..." << std::endl;
    int rc = std::system(compileCmd.c_str());
    return rc == 0 && std::filesystem::exists(executablePath);
}

bool runMoCHyAndParseRuntime(const std::string& executablePath, const std::string& csvInputPath, double& runtimeSeconds) {
    const std::string cmd = "\"" + executablePath + "\" " + std::to_string(kMoCHyThreads) + " \"" + csvInputPath + "\"";
...
bool writeOutput(const std::vector<std::tuple<std::string, std::string, int, double>>& rows) {
    std::ofstream out("escher_vs_mochy_e_deletevary.txt");
    if (!out.is_open()) {
        std::cerr << "Error: cannot write output file" << std::endl;
        return false;
    }
    out << "Mode Dataset DeletePercentage TimeSeconds\n";
...
int main() {
    const std::string mochyExe = "../../../MoCHy-master/exact_cli_par";
    if (!ensureMoCHyExecutable(mochyExe)) {
        std::cerr << "Failed to compile or find MoCHy executable at " << mochyExe << std::endl;
        return 1;
    }
    ...
    for (const auto& ds : kDatasets) {
        ...
        int effectiveTotalChanges = std::min(kTargetTotalChanges, std::max(1, 2 * static_cast<int>(h2v.size())));

        for (int delPct : kDeletePercentages) {
            ...
            params.deletionPercentage = static_cast<double>(delPct);
            params.insertionPercentage = static_cast<double>(100 - delPct);
            ...
            rows.push_back({"ESCHER", ds.label, delPct, escherSeconds});

            auto compactChanged = compactNonEmptyHyperedges(updatedH2V);
            const std::string csvPath = "tmp_" + ds.label + "_del" + std::to_string(delPct) + ".csv";
            if (!writeMoCHyCsv(compactChanged, csvPath)) return 1;

            double mochySeconds = 0.0;
            if (!runMoCHyAndParseRuntime(mochyExe, csvPath, mochySeconds)) {
                std::cerr << "Failed to run MoCHy-E for " << ds.label << " delete=" << delPct << std::endl;
                return 1;
            }
            rows.push_back({"MoCHy", ds.label, delPct, mochySeconds});
        }
    }

    if (!writeOutput(rows)) return 1;
    std::cout << "figure8 benchmark complete." << std::endl;
    return 0;
}
