...
bool writeOutput(const std::vector<std::tuple<std::string, std::string, std::string, double>>& rows) {
    std::ofstream out("escher_vs_mochy_e_timing.txt");
    if (!out.is_open()) {
    ...
    for (const auto& ds : kDatasets) {
        ...
        for (const auto& changed : kChangedEdges) {
            ...
            if (!runMoCHyAndParseRuntime(mochyExe, csvPath, mochySeconds)) {
                std::cerr << "Failed to run MoCHy-E for " << ds.label << " changed=" << changed.second << std::endl;
                return 1;
            }
            rows.push_back({"MoCHy", ds.label, changed.second, mochySeconds});
        }
    }

    if (!writeOutput(rows)) return 1;
    std::cout << "figure7 benchmark complete." << std::endl;
    return 0;
}
