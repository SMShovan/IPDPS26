...
int main(int argc, char *argv[])
{
	...
	if (argc < 3){
		cerr << "Usage: " << argv[0]
		     << " <dataset> <delta> [threads=8] [graph_file_override] [result_file_override]" << endl;
		return 1;
	}
	...
	int threads = 8;
	if (argc >= 4) threads = max(1, stoi(argv[3]));
	omp_set_num_threads(threads);

	// File path
	string graphFile = "../data/" + dataset + "/" + dataset + ".txt";
	string writeFile = "../results/" + dataset + "_" + to_string(delta) + "_thymeP.txt";
	...
	for (int end = 0; end < E; end++){
		...
		#pragma omp parallel
		{
			vector<long long> local_counts(96, 0);
			long long local_valid = 0;
			#pragma omp for schedule(static)
			for (int ins_i = 0; ins_i < (int)instances.size(); ++ins_i){
				const hmotif& instance = instances[ins_i];
				...
				local_counts[motif_index_bc] += bc_cnt;
				local_counts[motif_index_cb] += cb_cnt;
				if (bc_cnt or cb_cnt) local_valid++;
			}
			#pragma omp critical
			{
				for (int m = 0; m < 96; ++m) motif2count[m] += local_counts[m];
				valid_local += local_valid;
			}
		}
		valid_instance_cnt += valid_local;
